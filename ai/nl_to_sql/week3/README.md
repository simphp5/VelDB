...
- `domain_detector.py` – Inspects table and column names from the provided schema to dynamically determine the database domain (e.g., retail, school, hospital, finance, unknown).
- `menu_generator.py` – Suggests context-appropriate dashboard menu items corresponding to the detected database domain.
- `chart_suggester.py` – Interprets column names (like date keywords) and row values (like numerics) to recommend a suitable chart visualization format (e.g., bar, line, pie, scatter) based on simple heuristics.

### New API Endpoints

**1. POST `/detect_domain`**
Detects the domain of the provided schema.
...
**2. POST `/suggest_menus`**
Generates suggested dashboard menus for the given domain.
...
{
  "domain": "retail",
  "menus": ["Customers", "Products", "Orders", "Sales", "Inventory"]
}
```
