...
from menu_generator import generate_menus
...
@app.post("/detect_domain", response_model=DomainDetectResponse)
async def api_detect_domain(request: DomainDetectRequest):
    try:
        domain = detect_domain(request.schema_data)
        return DomainDetectResponse(domain=domain)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/suggest_menus", response_model=SuggestMenusResponse)
async def api_suggest_menus(request: SuggestMenusRequest):
    try:
        menus = generate_menus(request.domain)
        return SuggestMenusResponse(domain=request.domain, menus=menus)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/suggest_chart", response_model=SuggestChartResponse)
async def api_suggest_chart(request: SuggestChartRequest):
    try:
        chart_type = suggest_chart(request.columns, request.rows)
        return SuggestChartResponse(chart_type=chart_type)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
